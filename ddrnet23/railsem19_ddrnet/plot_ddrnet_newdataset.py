...
def plot_curves(data, model_label, outdir):
    ...
    _save_both(fig, outdir, "training_curves_ddrnet23_newdataset")
    plt.close(fig)
    ...
    fig.tight_layout(rect=[0, 0.08, 1, 1])
    _save_both(fig, outdir, "accuracy_ddrnet23_newdataset")
    plt.close(fig)
    ...
    fig.tight_layout(rect=[0, 0.08, 1, 1])
    _save_both(fig, outdir, "loss_ddrnet23_newdataset")
    plt.close(fig)
...
def main():
    ...
    ap.add_argument(
        "--csv",
        default="/data/pool/qmc-41b/ddrnet23_project/railsem19_ddrnet/checkpoints_new_seg_dataset_ignore67/metrics.csv",
        help="Path to metrics.csv",
    )
    ap.add_argument(
        "--logfile",
        default="/data/pool/qmc-41b/ddrnet23_project/railsem19_ddrnet/checkpoints_new_seg_dataset_ignore67/train.log",
        help="Path to train.log",
    )
    ap.add_argument("--label", default="DDRNet-23 Loss and Accuracy Graph", help="Plot title prefix")
    ap.add_argument(
        "--outdir",
        default="/data/pool/qmc-41b/ddrnet23_project/railsem19_ddrnet/checkpoints_new_seg_dataset_ignore67/plots_updated",
    ...
    try:
        if args.csv and os.path.isfile(args.csv):
            data = load_data(csv_path=args.csv, log_path=None)
        elif args.logfile and os.path.isfile(args.logfile):
            data = load_data(csv_path=None, log_path=args.logfile)
        else:
