...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="./checkpoints_new_seg_dataset")
...
def print_class_table(metrics, num_classes):
    log_info("+-------+--------+--------+")
    log_info("| Class |  IoU   |  Acc   |")
    log_info("+-------+--------+--------+")
    for cls in range(num_classes):
        iou = metrics["IoU"][cls] * 100.0
        acc = metrics["Acc"][cls] * 100.0
        log_info(f"| {cls:<5d} | {iou:>6.2f} | {acc:>6.2f} |")
    log_info("+-------+--------+--------+")
...
def main():
    ...
    log_info(
        ...
        f"num_classes={args.num_classes} image_size=({args.width},{args.height})"
    )
