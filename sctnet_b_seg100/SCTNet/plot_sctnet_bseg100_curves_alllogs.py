...
Works with:
- all .log files in a work_dir (important for resumed training)
- optional .log.json if explicitly provided
...
def parse_text_logs(log_paths):
    ...
    train_last = {
        ...
        for ep in last_train_loss_by_epoch
    }
    ...
    val_metrics = {
        ...
        for ep in set(val_aacc_by_epoch) | set(val_miou_by_epoch) | set(val_macc_by_epoch)
    }
...
def plot_and_save(epochs, train_loss, train_acc, val_loss, val_acc, title, outdir,
                  best_epoch=None, best_miou=None, early_bad=None):
    ...
    info_lines = []
    if best_epoch is not None and best_miou is not None:
        ax_loss.axvline(best_epoch, linestyle="--", alpha=0.35)
        info_lines.append(f"Best mIoU: {best_miou:.2f}% @ epoch {best_epoch}")

    if early_bad:
        last_ep = max(early_bad.keys())
        bad, pat = early_bad[last_ep]
        info_lines.append(f"EarlyStop bad={bad}/{pat} (last seen @ epoch {last_ep})")

    if info_lines:
        ax_loss.text(
            0.02, 0.98, "\n".join(info_lines),
            transform=ax_loss.transAxes, va="top", ha="left", fontsize=11,
            bbox=dict(boxstyle="round", fc="white", ec="none", alpha=0.75),
        )

    lines = [l_train_acc, l_val_acc, l_train_loss, l_val_loss]
    labels = [l.get_label() for l in lines]
    fig.legend(lines, labels, loc="upper center", bbox_to_anchor=(0.5, -0.10), ncol=2, frameon=False)
    fig.tight_layout(rect=[0, 0.08, 1, 1])
    fig.savefig(os.path.join(outdir, "training_curves.png"), dpi=200)
    plt.close(fig)

    # Accuracy only
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(epochs, train_acc, linestyle=":", marker="o", label="Training Acc")
    ax.plot(epochs, val_acc, linestyle="-.", marker="x", label="Validation Acc")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Accuracy [%]")
    ax.set_ylim(0, 100)
    ax.set_title(title.replace("Loss and Accuracy", "Accuracy"))
    ax.grid(True, which="both", linestyle="--", alpha=0.3)
    ax.legend(loc="best", frameon=False)
    fig.tight_layout()
    fig.savefig(os.path.join(outdir, "accuracy.png"), dpi=200)
    plt.close(fig)

    # Loss only
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(epochs, train_loss, linestyle="--", marker="^", label="Training Loss")
    ax.plot(epochs, val_loss, linestyle="-", marker="s", label="Validation Loss")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Loss")
    ax.set_title(title.replace("Loss and Accuracy", "Loss"))
    ax.grid(True, which="both", linestyle="--", alpha=0.3)
    ax.legend(loc="best", frameon=False)
    fig.tight_layout()
...
def main():
    ...
    ap.add_argument("--log", default=None, help="Single .log file")
    ap.add_argument("--json", default=None, help="Single .log.json file (preferred)")
    ...
    if args.json and os.path.isfile(args.json):
        ...
        if not log_paths:
            raise FileNotFoundError("Could not find .log files.")
        train_last, val_loss, val_metrics, early_bad = parse_text_logs(log_paths)
    ...
    plot_and_save(
        ...
        best_epoch=best_ep, best_miou=best_miou,
        early_bad=early_bad
    )

    print("[OK] Saved in:", os.path.abspath(args.outdir))
    print(" - training_curves.png")
    print(" - accuracy.png")
    print(" - loss.png")
    print(f"[INFO] Epoch range plotted: {epochs[0]} -> {epochs[-1]}")
