...
def parse_logjsons(workdir):
            ...
            for line in f:
                ...
                if mode == "train":
                    ...
                    if epoch not in train_by_epoch:
                        train_by_epoch[epoch] = {"losses": [], "accs": []}
                    if loss is not None:
                        train_by_epoch[epoch]["losses"].append(float(loss))
                    if acc is not None:
                        train_by_epoch[epoch]["accs"].append(float(acc))
    ...
    return files, train_by_epoch, val_by_epoch


def parse_plain_logs_for_val_loss(workdir):
    files = sorted(glob.glob(os.path.join(workdir, "*.log")))
    if not files:
        raise FileNotFoundError(f"No .log files found in {workdir}")
    ...
    return files, val_loss_by_epoch


def build_dataset(workdir):
    json_files, train_by_epoch, val_by_epoch = parse_logjsons(workdir)
    log_files, val_loss_by_epoch = parse_plain_logs_for_val_loss(workdir)

    all_epochs = sorted(set(train_by_epoch.keys()) | set(val_by_epoch.keys()) | set(val_loss_by_epoch.keys()))
    if not all_epochs:
        raise RuntimeError("No epoch records found.")
    ...
    return {
        ...
        "sources_json": json_files,
        "sources_log": log_files,
    }
...
def main():
    ...
    data = build_dataset(args.workdir)
    plot_curves(data, args.label, outdir, args.stem_prefix)

    print("[INFO] JSON sources used:")
    for s in data["sources_json"]:
        print("  ", s)
    print("[INFO] LOG sources used:")
    for s in data["sources_log"]:
        print("  ", s)
