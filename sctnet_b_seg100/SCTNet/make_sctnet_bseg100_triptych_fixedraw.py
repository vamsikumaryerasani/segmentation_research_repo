...
def find_gt_mask_by_basename(sample_path, gt_dir):
...
def find_raw_image_by_basename(sample_path, raw_dir):
    """
    For augmented samples like:
      rs00029_bri0p5.jpg
      rs00029_con0p7.jpg
    use the original raw frame:
      rs00029.jpg
    from raw_dir.
    """
    base = os.path.splitext(os.path.basename(sample_path))[0]
...
def main():
    ...
    ap.add_argument("--sample-dir", required=True, help="folder used only to choose sample filenames")
    ap.add_argument("--raw-dir", required=True, help="folder containing the true original RGB images")
    ...
    for p in [args.sample_dir, args.raw_dir, args.gt_dir]:
        if not os.path.isdir(p):
            raise SystemExit(f"[ERROR] directory not found: {p}")

    samples = list_images(args.sample_dir)[:args.max_images]
    if not samples:
        raise SystemExit(f"[ERROR] No sample images found in: {args.sample_dir}")
    ...
    for sample_path in samples:
        raw_path = find_raw_image_by_basename(sample_path, args.raw_dir)
        if raw_path is None:
            print("[WARN] Raw image not found for:", os.path.basename(sample_path))
            continue

        gt_path = find_gt_mask_by_basename(sample_path, args.gt_dir)
        if gt_path is None:
            print("[WARN] GT not found for:", os.path.basename(sample_path))
            continue
        ...
        # IMPORTANT: inference on the augmented sample image, not the raw image
        result = inference_segmentor(model, sample_path)
        ...
        strip_color = make_triptych(raw, pred_col, gt_col)
        strip_gray  = make_triptych(raw, pred_gray, gt_gray)

        title = title_prefix_from_filename(sample_path)
        token = os.path.splitext(os.path.basename(sample_path))[0]
        ...
        n_done += 1
        print(f"[{n_done}/{len(samples)}] wrote {token}")
