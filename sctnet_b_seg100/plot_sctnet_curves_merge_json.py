...
def parse_all_logjson(workdir):
    files = sorted(glob.glob(os.path.join(workdir, "*.log.json")))
    if not files:
        raise FileNotFoundError(f"No .log.json files found in {workdir}")
    ...
    for fp in files:
        with open(fp, "r", errors="ignore") as f:
            for line in f:
                ...
                # Train records
                if mode == "train":
                    loss = row.get("loss", row.get("decode.loss_ce", None))
                    acc = row.get("decode.acc_seg", None)
                    if loss is not None or acc is not None:
                        rec = train_by_epoch.setdefault(epoch, {"losses": [], "accs": []})
                        if loss is not None:
                ...
                # Val records
                    ...
                    if aacc is not None or miou is not None or macc is not None:
                        val_by_epoch[epoch] = {
                            "aAcc": float(aacc) * 100.0 if aacc is not None and aacc <= 1.5 else (float(aacc) if aacc is not None else None),
                            "mIoU": float(miou) * 100.0 if miou is not None and miou <= 1.5 else (float(miou) if miou is not None else None),
                            "mAcc": float(macc) * 100.0 if macc is not None and macc <= 1.5 else (float(macc) if macc is not None else None),
                            "val_loss": float(row.get("val_loss")) if row.get("val_loss", None) is not None else None,
                        }

                # Hook-only val_loss sometimes lands without mode
                if "val_loss" in row and epoch is not None:
                    vrec = val_by_epoch.setdefault(epoch, {})
                    vrec["val_loss"] = float(row["val_loss"])

    all_epochs = sorted(set(train_by_epoch.keys()) | set(val_by_epoch.keys()))
    if not all_epochs:
        raise RuntimeError("No epoch records found in log.json files.")

    epochs, train_loss, train_acc, val_loss, val_acc, miou, macc, aacc = [], [], [], [], [], [], [], []
    ...
    for ep in all_epochs:
        ...
        val_loss.append(vrec.get("val_loss", None))
    ...
    best_idx = max(valid_miou, key=lambda x: x[1])[0] if valid_miou else 0
    ...
    return {
        ...
        "best_miou": miou[best_idx] if miou[best_idx] is not None else None,
        "sources": files,
    }
...
def main():
    ...
    data = parse_all_logjson(args.workdir)
